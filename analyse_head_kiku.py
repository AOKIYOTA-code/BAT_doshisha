...
path = r"C:\Users\yota-\Documents\Head_Pulse"
Env = "Env1"
Bat = "2899" 
no = "no2"
other = "_1"
...
timing_round3 = timing.round(3)
pulse_path = bat_df[bat_df["Time"].isin(timing_round3)]
...
ato_atte = 2.4*(distances_df-0.1)     #吸収減衰
...
def fit(func, x, param_init):
    ...
    try:
        ...
        y = func(sample_x, *popt)  # sample_xに最適化されたパラメータで関数を適用
        success = True
    except RuntimeError:
        print("Optimal parameters not found: Number of calls to function has reached maxfev.")
        popt, perr, y = param_init, None, func(sample_x, *param_init)  # 初期値でのフィットを使用
        success = False

    return y, popt, perr, success

# パラメータの初期値の設定
param_init = [80, 0, 20]  # y軸の最大, 平均, 標準偏差
sample_x = np.arange(-150, 150, 0.01)  # グラフ描画用の範囲
...
for col in range(num_cols):
    ...
    # combined_colの最大値の位置を取得
    max_index = np.argmax(combined_col)
    max_angle = angles_col[max_index]

    # 最大値の位置を基準に±75度の範囲でフィルタリング
    mask = (angles_col >= max_angle - 75) & (angles_col <= max_angle + 75)
    angles_col_filtered = angles_col[mask]
    combined_col_filtered = combined_col[mask]

    # NaNやinfを除去
    valid_mask = np.isfinite(angles_col_filtered) & np.isfinite(combined_col_filtered)
    angles_col_filtered = angles_col_filtered[valid_mask]
    combined_col_filtered = combined_col_filtered[valid_mask]

    # データが空でないか確認
    if angles_col_filtered.size == 0 or combined_col_filtered.size == 0:
        print(f"Column {col+1}: No valid data points for fitting.")
        continue  # データが空の場合は次の列へ

    # パラメータの初期値をデータに基づいて設定
    param_init = [max(combined_col_filtered), np.mean(angles_col_filtered), np.std(angles_col_filtered)]

    # ガウスフィッティングに必要なデータの設定
    data_gauss_func = np.array([angles_col_filtered, combined_col_filtered])

    # フィッティングの実行
    result = fit(gauss_func, data_gauss_func, param_init)
    y_fit, popt, perr, success = result

    # フィッティング成功時のみR²を計算
    if success:
        X = data_gauss_func[0]
        Y = data_gauss_func[1]
        Y_fit = gauss_func(X, *popt)
    ...
    ax.plot(sample_x, y_fit, label='Fit', color='red')
    ax.legend()
