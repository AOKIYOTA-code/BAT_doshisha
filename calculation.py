...
def rotation(x: list, y: list):
    ...
    """
    # ±40°をラジアンで等間隔に分割
    rads_base = [np.deg2rad(round(j*0.01, 1)) for j in range(-4000, 4032, 32)]
    rot_x = []
    rot_y = []
    length = 5
    for i in range(len(x)-1):
        ...
        # 進行方向thetaを中心に±40°
        rads = [theta + rad for rad in rads_base]
        ...
        for x1, y1 in zip(rot_x[i], rot_y[i]):  ### 251
            ...
            for j in range(len(obs_x_list)):
                for k in range(len(obs_x_list[0]) - 1):
                    ...
                    if j==0:#壁の時
                        if r > first_dis:##壁より手前に障害物がある場合
                            pass
                        elif r == np.nan:##障害物がない場合
                            pass
                        else:##壁と交点がある場合
                            #rを座標にする
                            wall_cross_point_x=x0+(x1-x0)*r
                            wall_cross_point_y=y0+(y1-y0)*r
                            #rが壁に垂直か
                            #1.2:壁の端
                            #3:正射影　垂線との交点
                            if x2-0.015 < wall_cross_point_x < x2+0.015 and y2-0.015 < wall_cross_point_y < y2+0.015:
                                first_dis = r
                                
                            elif x3-0.015 < wall_cross_point_x < x3+0.015 and y3-0.015 < wall_cross_point_y < y3+0.015:
                                first_dis = r
                                
                            elif x0-0.015 < wall_cross_point_x < x0+0.015 or y0-0.015 < wall_cross_point_y < y0+0.015:
                                first_dis = r
            ...
            if first_dis == 10:
                first_dis = 2
            cross_subdistance.append(first_dis)#251次元のリスト
            cross_subdistance2=get_min_distance(cross_subdistance,tolerance=0.01)
            # print("cross_subdistance2:",cross_subdistance2)
        cross_alldistance.append(cross_subdistance2)#251次元*251
        # print("cross_alldistance:",cross_alldistance)
