...
path = r"C:\Users\yota-\OneDrive - 同志社大学\PO-MC-DHVRNN-sakai\result\20251127_kiku"
...
for episode in range(len(data[0][0][0][0])):
    ...
    # train_pulseが1のときのtest_pulseの値を抽出
    for i in range(len(train_pulse)):
        if train_pulse[i] >= 0.5:
            test_pulse_when_train_is_1.append(test_pulse[i])
    ...
    #print(data[0][0][step][0][episode][5])
    #print("===============")
...
# --- グラフの色の設定 ---
# ユビ
# VIOLIN_FACE_COLOR = 'lightblue'     # バイオリンプロットの塗りつぶし色
# VIOLIN_EDGE_COLOR = 'royalblue'     # バイオリンプロットの枠線の色
# BOX_PLOT_COLOR = 'k'                # 箱ひげ図の枠線とひげの色 (k=black)
# MEDIAN_COLOR = 'red'             # 箱ひげ図の中央値の線の色
# SCATTER_COLOR = '#1f77b4'           # 実際のデータ点の色 (matplotlibのデフォルト青)
# -------------------------

# キク
VIOLIN_FACE_COLOR = 'pink'          # バイオリンプロットの塗りつぶし色
VIOLIN_EDGE_COLOR = 'red'           # バイオリンプロットの枠線の色
BOX_PLOT_COLOR = 'k'       # 箱ひげ図の枠線とひげの色
MEDIAN_COLOR = 'red'                # 箱ひげ図の中央値の線の色
SCATTER_COLOR = 'salmon'            # 実際のデータ点の色
# -------------------------
...
# バイオリンプロットの作成
fig_violin, ax_violin = plt.subplots(figsize=(6, 8)) # figsize=(横, 縦)でサイズを指定
ax_violin.set_title('Violin Plot (All Episodes)')
ax_violin.set_ylabel('Predicted Pulse Probability')

ax_violin.set_ylim(Y_AXIS_MIN, Y_AXIS_MAX) # Y軸の範囲を設定
# バイオリンプロットと箱ひげ図を重ねて描画
violin_parts = ax_violin.violinplot([test_pulse_when_train_is_1], showmeans=False, showmedians=False, showextrema=False)

# バイオリンプロットの色の変更
for pc in violin_parts['bodies']:
    pc.set_facecolor(VIOLIN_FACE_COLOR)  # 塗りつぶしの色
    pc.set_edgecolor(VIOLIN_EDGE_COLOR)     # 枠線の色
    pc.set_alpha(0.8)            # 透明度

ax_violin.boxplot([test_pulse_when_train_is_1], showfliers=False,
                  patch_artist=True,  # 箱を塗りつぶし可能なオブジェクトとして描画
                  boxprops=dict(facecolor='none', edgecolor=BOX_PLOT_COLOR),  # facecolorで塗りつぶし、edgecolorで枠線の色を指定
                  widths=0.1, positions=[1], medianprops=dict(color=MEDIAN_COLOR), # 中央値の線の色を変更
                  whiskerprops=dict(color=BOX_PLOT_COLOR), # ひげの色を設定
                  capprops=dict(color=BOX_PLOT_COLOR))     # ひげの端のキャップの色を設定

# 実際のデータ点をプロット
x = np.random.normal(1, 0.04, size=len(test_pulse_when_train_is_1))
scatter_plot = ax_violin.scatter(x, test_pulse_when_train_is_1, alpha=0.3, color=SCATTER_COLOR)

ax_violin.set_xticks([])
# 凡例に表示する要素を明示的に指定する
handles, labels = ax_violin.get_legend_handles_labels()
ax_violin.legend(handles=[scatter_plot])
plt.savefig(os.path.join(path, 'violin_plot.pdf'))
plt.show()
