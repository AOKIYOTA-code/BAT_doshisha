...
if __name__ == "__main__":
    ...
    # test pickle load
    with open(
        os.path.dirname(game_files) + "/single_kiku.pkl",
        "rb",
    ) as f:
    ...
    # クラス不均衡対策のための重み計算 (pred_type == 2 の場合)
    if params["pred_type"] == 2:
        print("Calculating pos_weight for imbalanced dataset...")
        # train_loaderからデータを取得して計算
        ...
        pos_count = 0
        total_count = 0
        # Y_data (pulse flag) は入力データ(data)の特定の列にある
        # (batch, agents, time, feat)
        # in_sma=True の場合、5番目の特徴量がパルスフラグ
        pulse_feature_index = 5
        for data, _ in temp_loader:
            pulses = data[:, 0, :, pulse_feature_index] # agent 0
            pos_count += torch.sum(pulses)
            total_count += pulses.numel()
        
        neg_count = total_count - pos_count
        # 計算された重みを2で割る
        calculated_weight = (neg_count / pos_count if pos_count > 0 else 1.0) / 2.0
        params['pos_weight'] = torch.tensor(calculated_weight)
        print(f"Positive samples: {pos_count}, Negative samples: {neg_count}")
        print(f"Calculated pos_weight: {params['pos_weight']}")
    ...
    # pos_weightをGPUに転送
    if 'pos_weight' in params and args.cuda:
        params['pos_weight'] = params['pos_weight'].cuda()
