...
for idx, target_env in enumerate(target_env_list):
    ...
    for target_bat in target_bat_list:
        ...
        target_pulse_list = glob.glob(f"./rawdata_yubi/{env_name}/{os.path.split(target_bat)[-1]}/pulse/*.csv")
        target_path_list = glob.glob(f"{target_bat}/path/*.csv")
        bat_name = os.path.split(target_bat)[-1]
        
        # combineディレクトリが存在しない場合は作成
        combine_dir = f"{target_bat}/combine"
        if not os.path.exists(combine_dir):
            os.makedirs(combine_dir)
            print(f"Created directory: {combine_dir}")
        ...
        for target_pulse in target_pulse_list:
            ...
            for target_path in target_path_list:
                ...
                if pulse_fname in path_fname:
                    ...
                    pulse = pulse_df['Call_Time'].tolist()
                    pulse_trigger = pulse
