...
[fname3, fpath3]=uigetfile('*.bin', '26ch音声ファイルを開く');
...
% fseek(fid3,position, 'bof');
        ...
        cut_high2=round(peak_freq1-delta_freq/df);

        filetered_B2=filetered_B(1:cut_high2,:);

        [peak_power2 ,peak_freq2]=max(max(filetered_B2,[],2));
%        peak_freq=(peak_freq1+cut_low)*df;
        peak_freq=(peak_freq2+cut_low)*df;

        fprintf(fid1,'\t%5.5f\t%5.5f\t%5.5f',tim,peak_power2,peak_freq);
