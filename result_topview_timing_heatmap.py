...
path = r"C:\Users\yota-\OneDrive - 同志社大学\PO-MC-DHVRNN-sakai\result\20251126_yubi"
...
pp = PdfPages(os.path.join(path, 'topview_heatmap.pdf'))  ##########
...
####### kiku #######
    ...
    ax.plot(train_x_pre, train_y_pre, label='measured flight path',
            color='#1f77b4', zorder=2, linewidth=4,)
    
    # ヒートマップとして予測軌跡を描画
    points = np.array([test_x, test_y]).T.reshape(-1, 1, 2)
    segments = np.concatenate([points[:-1], points[1:]], axis=1)

    # 各線分の色を決定 (test_pulseの値をそのまま色情報として使う)
    # 線分の色は始点の色を採用
    colors = np.array(test_pulse[:-1])

    # LineCollectionを作成
    norm = Normalize(vmin=0, vmax=1)
    lc = LineCollection(segments, cmap='coolwarm', norm=norm) # 0:青, 1:赤
    lc.set_array(colors)
    lc.set_linewidth(4)
    lc.set_zorder(3)
    ...
    for i in range(len(train_pulse)):
        if train_pulse[i] >= 0.5:
            ax.scatter(train_x_pre[i], train_y_pre[i], label='measured pulse timing', facecolors='none', edgecolors='k', s=25, zorder=4)
        #if test_pulse[i] >= 0.5: 
    ...
    plt.tight_layout()
    ax.set_aspect('equal')
    fig.colorbar(line, ax=ax, label='Pulse Probability')
