...
#####TRAIN#####
episodes = 333 ###pulse_flag:212 dummy_flag:208
# data = np.loadtxt('./dataset/train/train.csv', delimiter=',')
data = pd.read_csv(f'./teshima/yubi_tim.csv', header=None)
data = data.values


alldata = data.reshape(1, episodes, step, 266)

with open(f'./teshima/yubi_tim.pkl', 'wb') as f:
