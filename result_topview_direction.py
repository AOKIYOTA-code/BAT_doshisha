...
path = r"C:\Users\yota-\OneDrive - 同志社大学\PO-MC-DHVRNN_aoki\result\20251119_pd_400epoch"
...
pp = PdfPages(os.path.join(path, 'topview_all.pdf')) ##########
...
for episode in range(len(data[0][0][0][0])):
    ...
    train_pulse = []
    test_pulse = []
    final_angle_diffs = [] # 最終放射角度の誤差を格納するリスト
    angle_diffs = []
    ...
    #print(data[1][0][step][0][episode][8])
...
####### kiku #######
    ...
    ax.plot(train_x_pre, train_y_pre, label='measured flight path',
            color='gray', zorder=2, linewidth=2)
    ...
    for i in range(len(train_x_pre)):
        ...
        if i < len(train_x_pre) - 1 and train_pulse[i] >= 0.5:
            ...
            # 矢印を描画
            dx_arrow_measured = np.cos(final_rad_measured) * 0.35
            dy_arrow_measured = np.sin(final_rad_measured) * 0.35
            ...
            ax.plot([train_x_pre[i], train_x_pre[i] + dx_arrow_measured], 
                    [train_y_pre[i], train_y_pre[i] + dy_arrow_measured], color='b', linewidth=1.8, zorder=3, linestyle='-')
            ...
            # 矢印を描画
            dx_arrow_predicted = np.cos(final_rad_predicted) * 0.35
            dy_arrow_predicted = np.sin(final_rad_predicted) * 0.35
            #ax.arrow(test_x[i], test_y[i], dx_arrow_predicted, dy_arrow_predicted, head_width=0.1, head_length=0.1, fc='r', ec='r', zorder=3)
            ax.plot([test_x[i], test_x[i] + dx_arrow_predicted], 
                    [test_y[i], test_y[i] + dy_arrow_predicted], color='r', linewidth=1.8, zorder=3)
            #print(final_rad_predicted)
            #print(final_rad_measured)
            ...
            # --- 最終的な放射角度の差を計算 ---
            # ラジアンから度数に変換
            final_deg_measured = np.rad2deg(final_rad_measured)
            final_deg_predicted = np.rad2deg(final_rad_predicted)
            final_angle_diff = abs(final_deg_predicted - final_deg_measured)
            # 角度差が180度を超えないように調整
            final_angle_diff = final_angle_diff % 360
            if final_angle_diff > 180:
                final_angle_diff = 360 - final_angle_diff
            final_angle_diffs.append(final_angle_diff)

    for i in range(len(train_pulse)):
        if train_pulse[i] >= 0.5:
            ax.scatter(train_x_pre[i], train_y_pre[i], label='measured pulse timing', color='k', s=10, zorder=4)
        if test_pulse[i] >= 0.5:
            ax.scatter(test_x[i], test_y[i], label='predicted pulse timing', color='k',edgecolor ='k' ,s=10, zorder=4)
    #print(test_pulse[i])
    ...
    pp.savefig(fig)
    plt.close(fig) # メモリを解放するためにFigureを閉じる

    # 最終的な放射角度の誤差の時間推移グラフを作成
    # if final_angle_diffs:
    #     # パルスが放射された時間ステップ（burn-in後）を取得
    #     pulse_steps = [i for i, p in enumerate(train_pulse) if p >= 0.5 and i < len(train_x_pre) - 1]

    #     # 新しいFigureを作成
    #     fig_err_final = plt.figure(figsize=(12, 6))
    #     ax_err_final = fig_err_final.add_subplot(111)

    #     ax_err_final.plot(pulse_steps, final_angle_diffs, marker='o', linestyle='-', color='purple')
    #     ax_err_final.set_title(f'Episode {episode+1}: Absolute Difference of Final Pulse Direction Angle over Time')
    #     ax_err_final.set_xlabel('Time Step (after burn-in)')
    #     ax_err_final.set_ylabel('Absolute Angle Difference (degrees)')
    #     ax_err_final.grid(True)
    #     ax_err_final.set_ylim(0, 180) # Y軸の範囲を0から180度に設定

    #     pp.savefig(fig_err_final) # 誤差グラフをPDFに保存
    #     plt.close(fig_err_final) # メモリを解放するためにFigureを閉じる

    # # 角度誤差の時間推移グラフを作成
    # if angle_diffs:
    #     # パルスが放射された時間ステップ（burn-in後）を取得
    #     pulse_steps = [i for i, p in enumerate(train_pulse) if p >= 0.5]

    #     # 新しいFigureを作成
    #     fig_err = plt.figure(figsize=(12, 6))
    #     ax_err = fig_err.add_subplot(111)

    #     ax_err.plot(pulse_steps, angle_diffs, marker='o', linestyle='-')
    #     ax_err.set_title(f'Episode {episode+1}: Angle Difference of Pulse Direction over Time')
    #     ax_err.set_xlabel('Time Step (after burn-in)')
    #     ax_err.set_ylabel('Angle Difference (degrees)')
    #     ax_err.grid(True)
    #     ax_err.set_ylim(0, 180) # Y軸の範囲を0から180度に設定

    #     pp.savefig(fig_err) # 誤差グラフをPDFに保存
    #     plt.close(fig_err) # メモリを解放するためにFigureを閉じる
